...
正式程式提交 `2345ee1eb`、簽署更新清單提交 `93b1fb40a` 已推送 `origin/main` 並由 Render 提供。R2 公開安裝檔 URL 為 `https://game-assets.rihdi.tw/desktop/launcher/releases/1.1.10/ONE-PIECE-Tabletop-Launcher-1.1.10-x64.exe`；HEAD 200、Range 206／MZ 檔首、完整 GET 的大小與 SHA-256 均與上述安裝檔一致。公開 `/desktop/launcher-release-v1.json` 與已簽署候選逐位元組一致：676 bytes，SHA-256 `3d27b490400e1879851dcf44f217c2fa021df75d08f17d73c246692e848c7dc1`，Ed25519 驗簽通過；1.1.9 查詢可更新，1.1.10 為最新版。公開商店預覽為 124 件，含新增四位房客；`LAUNCHER_CARD_SET` 未授權請求回 `bad secret`，證明正式路由存在且沒有寫入帳號。公開分發 144／144、Socket 10／10、下載頁 Chromium 38／38 通過；報告在 `D:\Codex_QA\launcher-room-1.1.10\`。正式玩家帳號的 PostgreSQL 購買、名片與房間交易未用真人帳號操作；本機 PGlite 交易測試不能代替此項驗收。LATTICE 官方 Status 仍為 `BLOCKED / CUSTOMER_DEPENDENCY_FILE_SET_CHANGED`，未宣稱本輪任務或圖譜已持久記錄。
...
來源 `b1a2e5705`、release `8b48ba1fd`、正式 `package-866f9d61a53eca35` 已於 21:42:05 生效；本機 UI 54、場景 126、瀏覽器 94＋切歌靜音 4、builder 41 及原 BGM／LAN 回歸通過。4,155 logical／3,838 unique 完整來源核對，13 個約 5.5 MB 更新 blobs 上傳及公開 SHA 通過；正式 runtime／metadata／43 程式 47 checks、兩個音樂室檔案 SHA 通過。27 份保護檔不變，18925 測試服務已關閉。

公開瀏覽器最終 97/97、errors 0。首輪 93 項成功，四項 MP3 因固定 260ms 窗口過短而重測；改成八秒內等實際進度，全部於 425ms 內通過，保留初輪報告及每項重測來源。只有 QA 的等待方式更正，產品檔不變；收尾以 `[skip render]` 提交。

## 海格抽選結果插畫（2026-09-21，已部署）

在正式 D 樹接入 24 種二選一事件、每種三版共 72 張 GPT 航海王角色插畫、隨機展示及實際道具／數值圖示，操作方與觀看方共用同一展示資料。修改 Board 主頁／主 JS，新增視覺 helper、CSS、獨立圖鑑、QA、素材來源紀錄、專用發布 builder 及 builder QA，更新桌面核心檔白名單及四份文件。隔離 npm start 18924；完整 Chrome／雙頁 LAN 210/210 checks、發布工具隔離 QA 24/24、原 LAN 刷新回歸通過。使用者後續已明確要求完成並部署，公開版本與保護證據見 [本次紀錄](BOARD_SEA_EVENT_REVEAL_20260921.md)。

來源 `f66dc1fce`、release `b50ae945a`，Board `package-b3342dbd8b206231` 已於 20:29:44 公開切換。完整來源 4,153 logical／3,836 unique 核對；新增 76 blobs 約 26.2 MB，R2 76、正式程式／metadata 45、新圖 72、公開瀏覽器 91 項皆通過，27 份保護檔不變。收尾文件以 `[skip render]` 提交。網頁重新整理；桌面更新後重開。

## 航海選單與攻擊特效（2026-09-21，已部署）

右上操作收合成三橫線選單；命中圖集由約 200–250ms 延長至單擊 700ms、連擊每下 420ms，切磋長連擊等完整演出後關閉。修改 Board 兩個 HTML、三支 JS 與五支 QA，更新四份專案文件。隔離 npm start 18923，選單32、快速航行20、FX Chrome116及雙頁LAN通過；切磋／部署結果見 [完整紀錄](BOARD_MENU_PACING_20260921.md)。

正式 release `b2a05a028`／Board `package-a2d57271de06415c` 已於 16:44 通過 43 項公開校驗與三尺寸選單驗收。網頁重新整理；桌面版下載約 5.2 MB 更新後重開。

## 敵人島勝利後進島修復（2026-09-21，已部署）
...
來源 `9b2965bb1`、release `f01b491f6`，Board `package-0f54c185d7d65dcb` 已於 15:52 公開驗證 43 項通過。修前真 LAN 重現，修後三案例、89 狀態檢查、兩頁刷新／交棒通過；R2 完整來源 4,079 檔與新增兩個 blob SHA 通過，27 份保護檔不變。收尾文件以 `[skip render]` 提交。
...
最終 102 圖集／816 格、487 音效完成；Chrome 100/100、播放器 38、整合 13、觀戰 41、傳輸 58 與原受擊順序 QA 通過。修正傷害數字圖層，21 次真音效播放全接受，24 個正式資料檔 SHA256 不變。桌面程式白名單增加兩支核心 JS；候選套件工具只寫獨立輸出目錄，589 新媒體保留逐檔 SHA。同步四份專案文件與來源／驗收報告。
...
release cf5cf4e7／Board package-d5357a5866ea75d8 已生效，23:47 公開完整性 41 checks 通過；R2 五份新檔 size/SHA 通過。正式選人 UI 35 候選 × 三尺寸，0 errors。832 項任務、20 項快速航行、45 項戰鬥提示及多人回歸均通過。此次收尾只補文件，提交使用 [skip render]。
