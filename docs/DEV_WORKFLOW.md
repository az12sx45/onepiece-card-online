...
- 相容邊界：沒有更動卡牌／Board 規則、角色或道具 ID、Socket.IO event 名稱、localStorage key、持久存檔 schema 或 `BOARD_GAME_STATE`。C 槽在最後檢查時僅剩約 31 MiB，這仍可能讓 Windows／Electron 的小型帳號狀態、記錄或其他系統操作失敗；本次已將可安全移動的遊戲快取改放 D 槽，但沒有刪除任何使用者檔案。
