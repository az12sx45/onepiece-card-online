...
const BASELINE = '9a74f0467cde382c773c293403521bbe28c1b71a';
...
const GENERATOR = 'water-seven-ship-relief-cors-release-v2';
const NEW_PROGRAMS = [];
const BATTLE_PROGRAMS = ['board_water_seven.html', 'js/board_water_seven_depth.js'];
...
function createBuilder(root = ROOT, baselineRef = BASELINE) {
  ...
  function collect(createdAt) {
    ...
    assert.deepEqual(config, expectedConfig, 'Program allowlist must remain unchanged.');
    assert.equal(priorConfig.games.board.programFiles.length, 53, 'Expected 53 baseline Board programs.');
    ...
    assert.equal(previous.totalFiles, 6363, 'Expected all 6,363 baseline Board records.');
    ...
    assert.deepEqual(changedNames, REQUIRED_PROGRAMS, 'Exactly four reviewed texture-loader and cache-version programs must change.');
    const added = manifest.assets.filter(asset => !oldByPath.has(asset.path)).map(asset => asset.path);
    assert.deepEqual(added, NEW_PROGRAMS, 'No new package paths may be added.');
    assert.equal(manifest.totalFiles, 6363, 'Candidate must preserve all 6,363 existing paths.');
...
if (require.main === module) {
  try { formal.main(); } catch (error) { console.error(`WATER_SEVEN_SHIP_CORS_RELEASE=FAIL ${error.message}`); process.exitCode = 1; }
}
