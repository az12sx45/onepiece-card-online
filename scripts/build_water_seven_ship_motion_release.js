...
const BASELINE = 'e8bfcad5f076189892e11ce02e8ece790cf7c663';
...
const GENERATOR = 'water-seven-ship-motion-release-v3';
const NEW_PROGRAMS = [];
...
function createBuilder(root = ROOT, baselineRef = BASELINE) {
  ...
  function collect(createdAt) {
    ...
    assert.deepEqual(changedNames, REQUIRED_PROGRAMS, 'Exactly four reviewed motion-strength and cache-version programs must change.');
...
if (require.main === module) {
  try { formal.main(); } catch (error) { console.error(`WATER_SEVEN_SHIP_MOTION_RELEASE=FAIL ${error.message}`); process.exitCode = 1; }
}
