...
// Scoped, offline release for the 24 sea-event scenes (three images each).
...
const BASELINE = '6c2e4f59d25eb7f5803894ef834d6a318da562f3';
...
const GENERATOR = 'board-sea-event-release-v1';
const PREFIX = 'images/board/sea_event_reveal/v2/';
const NEW_PROGRAMS = ['css/board_sea_event_reveal.css', 'js/board_sea_event_visuals.js'];
const CHANGED_PROGRAMS = ['board_game.html', 'js/board_game.js'];
const KEYS = ['supply-rescue', 'merchant-dividend', 'black-market', 'hull-damage', 'naval-inspection', 'tailwind', 'helpful-current', 'reverse-waves', 'storm-shelter', 'crosswind', 'cold-fog', 'drifting-chests', 'route-ticket-chest', 'charm-chest', 'picked-chest', 'fake-chest', 'wreck-vault', 'ancient-relic-chest', 'medical-ship', 'spirit-tonic', 'medical-kit', 'medicine-side-effect', 'spirit-drain', 'toxic-reaction'];
...
function imagePaths(helperBytes) {
  ...
  assert.ok(Array.isArray(definitions), 'Sea visual helper definitions are missing.');
  assert.deepEqual(definitions.map(item => item.key).sort(comparePaths), [...KEYS].sort(comparePaths), 'Sea helper must define exactly the reviewed 24 events.');
  for (const item of definitions) assert.deepEqual(item.images, [1, 2, 3].map(n => `${PREFIX}${item.key}-${n}.webp`), `Expected three V2 variants: ${item.key}`);
...
function createBuilder(root = ROOT, baselineRef = BASELINE) {
  ...
  function collect(createdAt) {
    ...
    assert.deepEqual(config, expectedConfig, 'Config may only add the two sea presentation programs.');
    assert.equal(priorConfig.games.board.programFiles.length, 39, 'Expected 39 baseline Board programs.');
    assert.equal(config.games.board.programFiles.length, 41, 'Expected 41 candidate Board programs.');
    ...
    assert.equal(retained.length, 4040, 'Expected 4,040 retained Board media records.');
    ...
    assert.deepEqual(trackedMedia, mediaNames, 'Committed V2 media inventory has missing or unexpected paths.');
    assert.deepEqual(listFiles(path.join(root, 'public', PREFIX)).map(name => PREFIX + name), mediaNames, 'V2 directory has missing or unexpected files.');
    ...
    assert.deepEqual(changed.map(asset => asset.path), CHANGED_PROGRAMS, 'Only board_game.html and js/board_game.js may change among existing programs.');
    const added = manifest.assets.filter(asset => !oldByPath.has(asset.path)).map(asset => asset.path);
    assert.deepEqual(added, [...NEW_PROGRAMS, ...mediaNames].sort(comparePaths), 'Only two new programs and 72 new images may be added.');
    assert.equal(manifest.totalFiles, previous.totalFiles + 74, 'Candidate file count');
...
if (require.main === module) {
  try { formal.main(); } catch (error) { console.error(`BOARD_SEA_EVENT_RELEASE=FAIL ${error.message}`); process.exitCode = 1; }
}
