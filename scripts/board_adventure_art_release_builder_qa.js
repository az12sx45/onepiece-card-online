...
const builder = require('./build_board_adventure_art_release');
const { canonicalJson, sha256Bytes, comparePaths } = require('./desktop_program_package_common');
const ROOT = path.resolve(__dirname, '..');
const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'board-adventure-release-qa-'));
...
function main() {
  ...
  const helper = fs.readFileSync(path.join(ROOT, 'public/js/board_adventure_art.js'));
  ...
  write('public/js/board_adventure_art.js', helper);
  for (const relative of builder.NEW_PROGRAMS.filter(p => p !== 'js/board_adventure_art.js')) write(`public/${relative}`, '/* isolated fixture program */\n');
  ...
  const good = collect();
  check('exact-release-inventory', () => { assert.equal(good.inputs.programs.length, 46); assert.equal(good.inputs.media.length, 54); assert.equal(good.inputs.changedAssets.length, 65); assert.equal(good.manifest.totalFiles, 4212); });
  check('all-4112-old-media-preserved', () => { const previous = JSON.parse(readBaseline(`public/${catalog.games.board.manifestPath}`)); const media = previous.assets.filter(asset => ['image', 'audio', 'video', 'font'].includes(asset.kind)); assert.equal(media.length, 4112); assert.deepEqual(good.manifest.assets.filter(asset => media.some(old => old.path === asset.path)), media); });
  ...
  check('unexpected-config-program-rejected', () => { const invalid = JSON.parse(JSON.stringify(config)); invalid.games.board.programFiles.push('zz-extra.js'); temporaryFile('config/desktop-program-packages-v1.json', canonicalJson(invalid), () => assert.throws(collect, /only add the three/), true); });
  check('unrelated-existing-program-change-rejected', () => temporaryFile('public/js/board_cards.js', 'unexpected program change\n', () => assert.throws(collect, /Only the eight/), true));
  ...
  check('missing-variant-in-helper-rejected', () => assert.throws(() => builder.imagePaths(Buffer.from('module.exports={definitions:[]};')), /reviewed 18 outcomes/));
