...
// Two-phase Service Worker release evidence for Card Depth visible V2, independently derived from V1. This script
...
const OUTPUT = path.join(ROOT, 'artifacts', 'card-depth-visible-v2', 'sw');
const BASELINE_FILE = path.join(OUTPUT, 'depth-v1-baseline.json');
const POSTDEPLOY_FILE = path.join(OUTPUT, 'visible-v2-postdeploy-report.json');
const PROFILE = path.join(OUTPUT, 'profiles', 'depth-v1');
const CACHE_NAME = 'op-card-v7.5';
const BASELINE_COMMIT = '40929bcade833267f48599135f9fdd29215f53d5';
const OLD_QUERY = 'v=20260907-depth-v1';
const NEW_QUERY = 'v=20260907-depth-visible-v2';
...
const OLD_REPRESENTATIVE_ASSETS = ['normal', 'enh', 'lux', 'lux-enh'].flatMap(variant =>
...
const REPRESENTATIVE_ASSETS = OLD_REPRESENTATIVE_ASSETS.map(resource => ({
  ...resource,
  path: resource.path.replace('/card-depth/v1/', '/images/card-depth/v1/')
}));
const BASELINE_NONCE_KEY = '__CARD_DEPTH_VISIBLE_V2_SW_BASELINE__';
...
async function fetchResource(base, resourcePath) {
  ...
  const response = await fetch(url, {
    ...
    headers: { 'user-agent': 'ONE-PIECE-Card-Depth-Visible-V2-SW-QA/1.0' }
  });
...
async function cacheSnapshot(page, resourcePaths) {
  return page.evaluate(async ({ cacheName, resourcePaths }) => {
    ...
    return {
      ...
      aliasDepthEntries: keys.map(request => new URL(request.url).pathname).filter(pathname => pathname.startsWith('/images/card-depth/v1/')),
      resources
    };
...
async function captureBaseline(chromium, options) {
  ...
  const report = {
    schemaVersion: 1,
    phase: 'capture-baseline',
    contract: 'Formal depth-v1 production and its old representative assets in an independent real Chrome profile; future aliases are observations only.',
  ...
  const code = await fetchResources(options.productionBase, [...OLD_CODE, ...OLD_REPRESENTATIVE_ASSETS, { path: '/sw.js', type: /^(?:application|text)\/javascript/i }]);
  report.directParity = validateDirect(code, OLD_CODE, BASELINE_COMMIT, check, 'depth-v1 production');
  report.assetParity = validateDirect(code, OLD_REPRESENTATIVE_ASSETS, BASELINE_COMMIT, check, 'depth-v1 baseline representative asset');
  const aliases = await fetchResources(options.productionBase, REPRESENTATIVE_ASSETS);
  report.aliasObservation = {
    contract: 'Informational only: alias HTTP 404 is expected before deployment, never a baseline failure.',
    allNotYetPublished: Object.values(aliases).every(resource => resource.status === 404),
    resources: cleanResources(aliases)
  };
  const swBlob = gitBlob(BASELINE_COMMIT, '/sw.js');
  check(code['/sw.js'].status === 200 && code['/sw.js'].sha256 === sha256(swBlob.body), 'depth-v1 production sw.js matches baseline commit');
  check(code['/game.html'].text.includes(`css/card-finish-v1.css?${OLD_QUERY}`) && code['/game.html'].text.includes(`js/card_finish_v1.js?${OLD_QUERY}`), 'baseline game.html references both formal depth-v1 resources');
  check(!code['/game.html'].text.includes(NEW_QUERY), 'baseline game.html does not yet reference visible-v2 code');
  ...
  try {
    ...
    await page.evaluate(({ key, nonce }) => localStorage.setItem(key, nonce), { key: BASELINE_NONCE_KEY, nonce });
    await page.goto(`${options.productionBase}/game.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    const baselinePaths = [...OLD_CODE, ...OLD_REPRESENTATIVE_ASSETS].map(resource => resource.path);
    // Fetch only old paths inside the controlled page, leaving future aliases unseeded.
    await page.evaluate(async paths => {
      for (const resourcePath of paths) {
        const response = await fetch(resourcePath);
        if (!response.ok) throw new Error('Baseline asset HTTP ' + response.status + ': ' + resourcePath);
        await response.arrayBuffer();
      }
    }, OLD_REPRESENTATIVE_ASSETS.map(resource => resource.path));
    await waitCached(page, baselinePaths);
    const controller = await controllerState(page);
    const cache = await cacheSnapshot(page, baselinePaths);
    check(controller.controlled && controller.controllerScript === `${options.productionBase}/sw.js`, 'dedicated depth-v1 profile is controlled by the production SW', controller);
    check(cache.names.includes(CACHE_NAME), `dedicated depth-v1 profile contains ${CACHE_NAME}`, cache.names);
    check(cache.aliasDepthEntries.length === 0, 'baseline profile contains no future images/card-depth alias cache entries', cache.aliasDepthEntries);
    for (const resource of [...OLD_CODE, ...OLD_REPRESENTATIVE_ASSETS]) {
      const cached = cache.resources[resource.path];
      check(cached.found && cached.status === 200 && cached.sha256 === code[resource.path].sha256, `baseline cache ${resource.path} matches direct depth-v1 production bytes`, cached);
    }
  ...
  process.stdout.write(`CARD_DEPTH_VISIBLE_V2_SW_QA=PASS phase=capture-baseline checks=${report.checkCount} profile=${PROFILE}\n`);
}

async function visibleDexCard(page, selector) {
  ...
  return root.evaluate(element => ({
    ...
    engaged: element.getAttribute('data-finish-engaged'),
...
async function postdeploy(chromium, options) {
  ...
  const report = {
    schemaVersion: 1,
    phase: 'postdeploy',
    contract: 'Direct release-byte parity plus preserved depth-v1 profile refresh under the unchanged production SW.',
    createdAt: new Date().toISOString(),
    productionBase: options.productionBase,
    releaseCommit,
  ...
  report.codeParity = validateDirect(live, NEW_CODE, releaseCommit, check, 'visible-v2 production');
  report.assetParity = validateDirect(live, REPRESENTATIVE_ASSETS, releaseCommit, check, 'visible-v2 representative asset');
  check(live['/game.html'].text.includes(`css/card-finish-v1.css?${NEW_QUERY}`) && live['/game.html'].text.includes(`js/card_finish_v1.js?${NEW_QUERY}`), 'deployed game.html references both visible-v2 query resources');
  check(live['/sw.js'].status === 200 && live['/sw.js'].sha256 === before.production['/sw.js'].sha256, 'production Service Worker source remains byte-identical to captured depth-v1 baseline');
  ...
  try {
    const page = await ensureController(context, options.productionBase);
    const nonce = await page.evaluate(key => localStorage.getItem(key), BASELINE_NONCE_KEY);
    check(nonce === before.profile.nonce, 'postdeploy opens the exact dedicated predeploy depth-v1 profile');
    await page.goto(`${options.productionBase}/game.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    const baseState = await visibleDexCard(page, '#cardDexPreviewBase');
    const enhancedState = await visibleDexCard(page, '#cardDexPreviewEnh');
    check(baseState.engaged === null && enhancedState.engaged === null, 'visible-v2 loads both visible composites without pointer engagement');
    ...
    const controller = await controllerState(page);
    check(controller.controlled && controller.controllerScript === `${options.productionBase}/sw.js`, 'preserved profile remains controlled by the production SW after visible-v2 deployment', controller);
    for (const resource of OLD_CODE.slice(1)) {
      const cached = cache.resources[resource.path];
      check(cached.found && cached.sha256 === before.production[resource.path].sha256, `preserved profile retains original V1 cache key ${resource.path}`, cached);
    }
    ...
    check(baseState.ready === 'ready' && enhancedState.ready === 'ready', 'preserved depth-v1 profile executes both normal and enhanced visible-v2 depth composites', { baseState, enhancedState });
    await page.screenshot({ path: path.join(OUTPUT, 'preserved-depth-v1-profile-visible-v2.png'), fullPage: false });
  ...
  process.stdout.write(`CARD_DEPTH_VISIBLE_V2_SW_QA=PASS phase=postdeploy checks=${report.checkCount} oldProfileRefresh=PASS\n`);
}
...
async function main() {
  ...
  if (options.help) {
    process.stdout.write([
      'Usage: node scripts/card_depth_visible_v2_sw_qa.js --phase capture-baseline|postdeploy [options]',
...
main().catch(error => {
  process.stderr.write(`CARD_DEPTH_VISIBLE_V2_SW_QA=FAIL ${error.stack || error}\n`);
  process.exitCode = 1;
});
