...
const start=source.indexOf('async function finishJudicialRaidBattle(battle)');
...
async function scenario(phase){
 ...
 const scope={Math,Date,JUDICIAL_RAID_PHASES:phases,JUDICIAL_PHASE_BONUS_POOL:[],state:{gameState:{round:1}},battlePlayer:()=>player,ensureJudicialRaidState:()=>raid={...raid,phaseClearedKeys:[...raid.phaseClearedKeys]},getIslandState:()=>island,
 ...
 const finish=vm.runInNewContext(`(${body})`,scope),pending=finish(battle);
 assert.equal(grants,1);assert.equal(notifications,1);assert.equal(finalized,0);assert.equal(next,0);assert.equal(raid.clearCount,0);assert.equal(battle.visualEvent.finalVictory,phase===5);
 ...
 await finish(battle);assert.equal(grants,1);assert.equal(notifications,1);results.push({name:`phase ${phase+1}: reentry cannot duplicate reward`,pass:true});
