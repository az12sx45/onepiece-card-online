...
const BASELINE = '073283a0d689fed1df6a5c6df69241501fa9e44f';
...
const GENERATOR = 'board-battle-recovery-release-v1';
const NEW_PROGRAMS = [];
const BATTLE_PROGRAMS = ['board_battle.html', 'css/board_character_depth.css', 'js/board_character_depth.js'];
...
function createBuilder(root = ROOT, baselineRef = BASELINE) {
  ...
  function collect(createdAt) {
    ...
    assert.equal(config.games.board.programFiles.length, 51, 'Expected 51 candidate Board programs.');
    ...
    assert.deepEqual(changedNames, REQUIRED_PROGRAMS, 'Exactly the five reviewed battle recovery and tilt programs must change.');
    ...
    assert.equal(manifest.totalFiles, 6361, 'Candidate must preserve all 6,361 old paths without adding paths.');
...
if (require.main === module) {
  try { formal.main(); } catch (error) { console.error(`BOARD_BATTLE_RECOVERY_RELEASE=FAIL ${error.message}`); process.exitCode = 1; }
}
