...
const BASELINE = '6c706729955b0e8efe1159f45c2285a78a347670';
...
const GENERATOR = 'board-hotkeys-fx-release-v1';
const NEW_PROGRAMS = ['css/board_hotkeys.css', 'js/board_hotkeys.js'];
const BATTLE_PROGRAMS = ['board_battle.html', 'js/board_battle.js', 'js/board_move_fx.js'];
...
function createBuilder(root = ROOT, baselineRef = BASELINE) {
  ...
  function collect(createdAt) {
    ...
    assert.deepEqual(config, expectedConfig, 'Config may only add the two hotkey programs.');
    assert.equal(priorConfig.games.board.programFiles.length, 46, 'Expected 46 baseline Board programs.');
    assert.equal(config.games.board.programFiles.length, 48, 'Expected 48 candidate Board programs.');
    ...
    assert.equal(previous.totalFiles, 4212, 'Expected all 4,212 baseline Board records.');
    ...
    assert.deepEqual(changedNames, REQUIRED_PROGRAMS, 'Exactly the five reviewed hotkey and battle programs must change.');
    const added = manifest.assets.filter(asset => !oldByPath.has(asset.path)).map(asset => asset.path);
    assert.deepEqual(added, NEW_PROGRAMS, 'Only two new hotkey programs may be added.');
    assert.equal(manifest.totalFiles, 4214, 'Candidate must preserve all 4,212 old paths and add two programs.');
...
if (require.main === module) {
  try { formal.main(); } catch (error) { console.error(`BOARD_HOTKEYS_FX_RELEASE=FAIL ${error.message}`); process.exitCode = 1; }
}
